...
def _extract_usage(response_obj: dict[str, Any]) -> dict[str, int]:
...
class OpenAICodexProvider(LLMProvider):
    ...
    async def chat(
        ...
        try:
            content, tool_calls, finish_reason, usage = await _request_codex(url, headers, body)
            ...
            self._log_token_usage(model, messages, llm_response)
            return llm_response
    ...
    def _log_token_usage(
        ...
        logger.info(
            "llm_tokens provider=openai_codex model={} input_tokens={} output_tokens={} total_tokens={} source={}",
            model,
            input_tokens,
            output_tokens,
            total_tokens,
    ...
    def _estimate_message_tokens(self, messages: list[dict[str, Any]], model: str) -> int:
        ...
        try:
            count = litellm.token_counter(model=resolved_model, messages=messages)
